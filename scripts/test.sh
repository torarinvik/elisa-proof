...
set +e
"$ROOT_DIR/build/elisa-proof" --json "$ROOT_DIR/examples/shared_borrow_calls.elisa" | python3 -c 'import json, sys; report = json.load(sys.stdin); assert report["status"] == "proved"; assert report["summary"]["semantic_errors"] == 0; assert report["summary"]["proven"] == report["summary"]["obligations"]; assert report["findings"] == []; assert report["replay"]["certificates"] == report["replay"]["replayed"]; assert report["replay"]["gaps"] == 0; nodes = report["kernel"]["nodes"]; children = report["kernel"]["children"]; kinds = {node["kind"] for node in nodes}; assert "resource-call-shared" in kinds; shared = [node for node in nodes if node["kind"] == "resource-call-shared"]; assert all(node["left"] == 0 and node["children_count"] == node["auxiliary"] * 2 for node in shared); formals = [nodes[child] for node in shared for child in children[node["children_start"] + node["auxiliary"]:node["children_start"] + node["children_count"]]]; assert formals; assert all(formal["kind"] == "resource-call-formal" for formal in formals); assert all(formal["operator"] in ("value", "external-shared") for formal in formals); assert any(formal["operator"] == "external-shared" for formal in formals)'
...
set +e
"$ROOT_DIR/build/elisa-proof" --json "$ROOT_DIR/examples/rejected_shared_borrow_calls.elisa" | python3 -c 'import json, sys; report = json.load(sys.stdin); assert report["status"] == "failed"; assert report["summary"]["semantic_errors"] == 0; assert report["replay"]["gaps"] == 0; findings = {(finding["kind"], finding["name"]) for finding in report["findings"]}; assert ("borrow-call-opaque", "recursive_mutate") in findings; assert ("borrow-call-opaque", "recursive_reference_return") in findings; assert ("resource-use-after-move", "lends_moved_value") in findings; assert ("borrow-call-opaque", "lends_shared_while_mutably_borrowed") in findings; assert not any(node["kind"] == "resource-call-shared" for node in report["kernel"]["nodes"])'
