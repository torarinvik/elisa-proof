...
"$ROOT_DIR/build/elisa-proof" --json "$ROOT_DIR/examples/dogfood_kernel_core.elisa" | python3 -c 'import json, sys; report = json.load(sys.stdin); assert report["status"] == "failed"; assert report["verification_state"] == "unsupported"; assert report["summary"]["proven"] == 19; assert [(f["name"], f["kind"]) for f in report["findings"]] == [("add_node", "contract-expression-unsupported")]; assert report["summary"]["declarations"] == 9; assert report["summary"]["obligations"] == 20; assert report["replay"]["gaps"] == 0; assert [goal["goal_id"] for goal in report["goals"]] == list(range(len(report["goals"]))); assert [certificate["certificate_id"] for certificate in report["certificates"]] == list(range(len(report["certificates"]))); assert all(goal["certificate_id"] is not None for goal in report["goals"]); assert all(goal["certificate_id"] < len(report["certificates"]) for goal in report["goals"])'
...
"$ROOT_DIR/build/elisa-proof" --json "$ROOT_DIR/src/proof/kernel_core.elisa" | python3 -c 'import json, sys; report = json.load(sys.stdin); assert report["status"] == "failed"; assert report["verification_state"] == "unsupported"; assert report["summary"]["proven"] == 6; assert [(f["name"], f["kind"]) for f in report["findings"]] == [("add_node", "contract-expression-unsupported")]; assert report["summary"]["obligations"] == 7; assert report["summary"]["semantic_errors"] == 0; assert report["replay"]["gaps"] == 0; assert report["kernel"]["independent_replay"] is True'
...
if [[ "$bounded_model_status" -ne 0 || "$dogfood_kernel_status" -ne 0 || "$dogfood_kernel_core_status" -ne 1 || "$rejected_dogfood_kernel_core_status" -ne 1 ]]; then
